...
async function errorHandler(req, res, e, options, routeHandle) {
    ...
    if (options.logAction) {
        if (options.logHandle)
            options.logHandle(routeHandle.control, routeHandle.routeData, result, true);
        else {
            // noinspection JSIgnoredPromiseFromCall
            routeHandle.control.writeLogAction({
                response: result,
                isError: true,
            });
        }
...
async function makeRouteHandle(req, res, options, args, skipError) {
    ...
    routeHandle.control.writeLogAction = async (data) => {
        data = Object.assign({
            user_id: routeHandle.routeData.session ? routeHandle.routeData.session.accountId : null,
            // name: options.summary ? options.summary : null,
            method: options.method,
            url: options.url,
            parameter: routeHandle.routeData.args,
            response: null,
            isError: false,
        }, data);

        if (data.parameter)
            delete data.parameter.authorization;
        

        let kq = await Db.log_action.create(
            data.user_id,
            data.method,
            data.url,
            data.parameter,
